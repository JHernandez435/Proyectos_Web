        ...
        if (count($objRespuesta) >= 1){
            $objJson = '{
                ...
                for ($i=0; $i < count($objRespuesta); $i++) { 
                    ...
                    $objBotones .= "<button id='btnEditar' class='btn btn-sm btn-primary' title='editar' data-toggle='modal' data-target='#modalEditarUsuario' idUsuario='".$objRespuesta[$i]["idusuario"]."' nombre='".$nombre."' apellido='".$apellido."' telefono='".$telefono."' imagen='".$imagen."'><i class='fa fa-edit'></i></button>";
