$(document).ready(function() {
    ...
    estado2 = urlParametros.get('transactionState');
    if (estado2 == "4") {
        actualizarProductos();
    }
    ...
    function actualizarProductos() {
        ...
        var listaCarritoVacia = [];
        var totalElementos = listaCarrito.length;

        if (totalElementos >= 1) {
            var objData = new FormData();
            objData.append("listaCarrito", JSON.stringify(listaCarrito));
            $.ajax({
                async: true,
                ...
                success: function(respuesta) {
                    ...
                    alert(respuesta.resultado[0]);


                    localStorage.setItem("productos", JSON.stringify(listaCarritoVacia));
